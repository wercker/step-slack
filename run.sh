...
if [[ $SLACK_NOTIFIER_CHANNEL == \#* ]]; then
  error "Please specify the channel without the '#'"
fi

# if no username is provided use the default - werckerbot
if [ ! -n "$SLACK_NOTIFIER_USERNAME" ]; then
  export SLACK_NOTIFIER_USERNAME=werckerbot
fi

# if no icon-url is provided for the bot use the default wercker icon
if [ ! -n "$SLACK_NOTIFIER_ICON_URL" ]; then
    export SLACK_NOTIFIER_ICON_URL="https://secure.gravatar.com/avatar/a08fc43441db4c2df2cef96e0cc8c045?s=140"
fi
